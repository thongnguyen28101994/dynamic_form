...
const FormUpdateSchool = ({ showModalSchool, showModalSchoolF }) => {
  ...
  const [dmLoaiChuongTrinh, setDmLoaiChuongTrinh] = useState([]);
  ...
  const getdata = async () => {
    await schoolAPI.getPGD().then((res) => {
      setPGDs(res.Result);
    });
    ...
    await schoolAPI.getLoaiChuongTrinh().then((s) => {
      setLoaiChuongTrinh(s.result);
      setSelectedLoaiChuongTrinh(s.result.map((s) => s.LoaiChuongTrinhID));
    });
    await schoolAPI
      .getDMLoaiChuongTrinh()
      .then((s) => setDmLoaiChuongTrinh(s.result));
  };
  ...
  const handleChangeLoaiChuongTrinh = async (id) => {
    
    if (
      loaiChuongTrinh.filter((s) => s.LoaiChuongTrinhID === id).length > 0
    ) {
      await schoolAPI
        .DeleteLoaiChuongTrinh(id)
        .catch((err) => message.error("Có lỗi xảy ra"));
    } else {
      await schoolAPI
        .InsertLoaiChuongTrinh([{ loaiChuongTrinhID: id }])
        .catch((err) => message.error("Có lỗi xảy ra"));
    }
    setSelectedLoaiChuongTrinh(selectedLoaiChuongTrinh.filter(s => s !== id))
  };
      ...
      >
        <Row>
          ...
          <Col span={24}>
            ...
            >
              <Input
                placeholder="Nếu không có Email thì nhập không có"
                onBlur={(e) => {
          ...
          <Col span={24}>
            ...
            >
              <Input
                placeholder="Nếu không có Website thì nhập không có"
                onBlur={(e) => {
          ...
          <Col span={24}>
            ...
            >
              <Input
                placeholder="Nếu không có Fax thì nhập không có"
                onBlur={(e) => {
          ...
          {/* <Col span={24}>
            <b>Loại Chương Trình</b> <br />
            <Checkbox.Group
              style={{ width: "100%" }}
              onChange={handleChangeLoaiChuongTrinh}
              // defaultValue={loaiChuongTrinh.map((s) => s.LoaiChuongTrinhID)}

              defaultValue={selectedLoaiChuongTrinh}
            >
              <Row>
                {dmLoaiChuongTrinh.map((s) => (
                  <Col span={8}>
                    <Checkbox value={s.LoaiChuongTrinhID}>
                      {s.TenLoaiChuongTrinh}
                    </Checkbox>
                  </Col>
                ))}
              </Row>
          ...
          <Row>
            {dmLoaiChuongTrinh.map((s) => (
              <Col span={8}>
                <Checkbox
                  onChange={() =>
                    handleChangeLoaiChuongTrinh(s.LoaiChuongTrinhID)
                  }
                  checked={selectedLoaiChuongTrinh.includes(
                    s.LoaiChuongTrinhID
                  )}
                  value={s.LoaiChuongTrinhID}
                >
                  {s.TenLoaiChuongTrinh}
                </Checkbox>
              </Col>
          ...
          <Col span={24}>
            ...
            >
              <Input
                ...
                onBlur={(e) => {
                  form.setFieldsValue({
                    url_kehoachchienluoc: e.target.value.trim(),
                  });
                }}
              />
            </Form.Item>
          ...
          <Col span={24}>
            ...
            >
              <Input
                placeholder="Nếu không có Website thì nhập không có"
                onBlur={(e) => {
                  form.setFieldsValue({ url_3congkhai: e.target.value.trim() });
                }}
              />
            </Form.Item>
          </Col>
          <Col span={24}>
            ...
            >
              <Input
                placeholder="Nếu không có Website thì nhập không có"
                onBlur={(e) => {
                  form.setFieldsValue({ url_khgiaoduc: e.target.value.trim() });
                }}
              />
            </Form.Item>
          </Col>
...
export default FormUpdateSchool;
